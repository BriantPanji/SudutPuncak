...
function SearchContent() {
  ...
  useEffect(() => {
    if (initialQuery) {
      handleSearch(initialQuery, selectedProvince, minElevation, sortBy, sortOrder);
    }
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex gap-2">
              ...
              <button
                ...
                className={`px-3 py-2.5 rounded-lg border transition-colors cursor-pointer ${
                  showFilters || selectedProvince || minElevation || sortBy !== 'relevance'
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {selectedProvince && (
              <span className="inline-flex items-center gap-1 px-3 py-1 bg-teal-900/50 text-teal-300 rounded-full text-sm border border-teal-700">
                ...
                <button onClick={() => { setSelectedProvince(''); setTimeout(() => handleSearch(searchQuery, '', minElevation, sortBy, sortOrder), 0); }} className="hover:text-white cursor-pointer">×</button>
              </span>
            )}
            {minElevation && (
              <span className="inline-flex items-center gap-1 px-3 py-1 bg-teal-900/50 text-teal-300 rounded-full text-sm border border-teal-700">
                Min: {minElevation} mdpl
                <button onClick={() => { setMinElevation(''); setTimeout(() => handleSearch(searchQuery, selectedProvince, '', sortBy, sortOrder), 0); }} className="hover:text-white cursor-pointer">×</button>
              </span>
            )}
            {sortBy !== 'relevance' && (
              <span className="inline-flex items-center gap-1 px-3 py-1 bg-teal-900/50 text-teal-300 rounded-full text-sm border border-teal-700">
                Urut: {sortBy === 'name' ? 'Nama' : sortBy === 'province' ? 'Provinsi' : 'Ketinggian'} ({sortOrder === 'asc' ? '↑' : '↓'})
                <button onClick={() => { setSortBy('relevance'); setTimeout(() => handleSearch(searchQuery, selectedProvince, minElevation, 'relevance', 'asc'), 0); }} className="hover:text-white cursor-pointer">×</button>
              </span>
            )}
          </div>
        )}
